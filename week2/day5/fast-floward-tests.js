...
describe('Flow Dapp Tests', async () => {
    ...
    describe('Fast Floward Tests', async () => {
        ...
        it(`removes a kittyitem from sale`, async () => {
            ...
            assert.equal(res1.result, 1, "User's Market collection should have length 1")
        ...
        it(`checks both admin and user have correct balances after purchase`, async () => {
            ...
            assert.equal(res2.result, 25.0, "Incorrect balance in account 1")

        })

        it(`burn kibble from the admin account and has the correct balance in the account`, async () => {
            let testData1 = {
                amount: "1.0",
                recipient: config.accounts[0]
            }
            ...
            assert.equal(res1.result, 4.0, "Admin has incorrect balance")

            assert.equal(res2.result, 29.0, "Incorrect total number of Kibble")
        })
    });
