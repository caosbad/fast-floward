...
transaction(recipient: Address, withdrawID: UInt64) {
    ...
    prepare(signer: AuthAccount) {
        ...
        self.receiverCollectionRef = getAccount(recipient).getCapability(KittyItems.CollectionPublicPath).borrow<&{NonFungibleToken.CollectionPublic}>()
                            ?? panic("Could not borrow capability from public collection")
        // self.receiverCollectionRef = getAccount(recipient).borrow<&{NonFungibleToken.CollectionPublic}>(from: KittyItems.CollectionPublicPath)
    }

    execute {

        // 3) withdraw the Kitty Item from the signer's Collection
        let NFTToken <- self.signerCollectionRef.withdraw(withdrawID: withdrawID)
        // 4) deposit the Kitty Item into the recipient's Collection
        self.receiverCollectionRef.deposit(token: <- NFTToken)
    }
}