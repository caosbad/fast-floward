...
transaction(recipient: Address, typeID: UInt64) {
    ...
    prepare(signer: AuthAccount) {
        ...
        self.receiver = recipientAccount.getCapability(KittyItems.CollectionPublicPath).borrow<&{NonFungibleToken.CollectionPublic}>()
                            ?? panic("Could not borrow capability from public collection")
        //self.receiver = recipientAccount.borrow<&{NonFungibleToken.CollectionPublic}>(from: KittyItems.CollectionPublicPath)
    }
