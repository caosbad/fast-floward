...
transaction(recipient: Address, metadata: {String: String}) {
    ...
    // the tenant
    let tenant: &{RegistryNFTContract.ITenantPrivate}
    let receiver: &RegistryNFTContract.Collection{NonFungibleToken.CollectionPublic}

    prepare(acct: AuthAccount) {

        self.tenant = acct.borrow<&{RegistryNFTContract.ITenantPrivate}>(from: RegistryNFTContract.TenantPrivatePath)
    ...
    execute {
        ...
        self.tenant.mintNFT(recipient: self.receiver, metadata: metadata)
    }
}